public class HeapSort{
    ...
    private static void downheap(Individual i[], int k, int N){
	    ...
	    public static void tunedSort(Individual i[]){
        int N = i.length;
    ...
    private static void tunedDownheap(Individual i[], int k, int N){
        Individual T = i[k - 1];
        
        while (k <= N/2) {
            int j = k + k;
            if ((j < N) && (i[j - 1].getFitness2() > i[j].getFitness2())) 
            j++;

            if (T.getFitness2() <= i[j - 1].getFitness2()) 
            break;
